...
if (!isset($_GET['id_artikel']) || !is_numeric($_GET['id_artikel'])) {
  echo "<script>alert('ID tidak valid!'); window.location='data_artikel.php';</script>";
  exit;
}
...
if (!$query || mysqli_num_rows($query) == 0) {
  echo "<script>alert('Data tidak ditemukan!'); window.location='data_artikel.php';</script>";
  exit;
}
...
<div class="flex max-w-7xl mx-auto mt-8 px-4">
  ...
  <main class="w-3/4 bg-white rounded shadow p-6 ml-6">
    <form action="proses_edit_artikel.php" method="post" class="space-y-6">
      <input type="hidden" name="id_artikel" value="<?= htmlspecialchars($data['id_artikel']); ?>">
      ...
      <div>
        ...
        <input type="text" id="nama_artikel" name="nama_artikel" required
               value="<?= htmlspecialchars($data['nama_artikel']); ?>"
      ...
      <div>
        ...
        <textarea id="isi_artikel" name="isi_artikel" rows="5" required
                  class="w-full p-2 border rounded focus:outline-none focus:ring focus:border-[#DD0000]"><?= htmlspecialchars($data['isi_artikel']); ?></textarea>
      </div>
